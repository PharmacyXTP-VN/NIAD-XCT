      ...
      <div className="min-h-screen bg-[#f0f2f5]">
        ...
        <section className="max-w-7xl mx-auto px-4 py-12 flex flex-col lg:flex-row gap-10">
          ...
          <div className="w-full lg:w-1/2 flex flex-col gap-4">
            ...
            <div className="flex gap-3 justify-center mt-2">
              {product.images?.front && <Image src={product.images.front} alt="Ảnh trước" width={80} height={60} className="object-contain w-20 h-14" />}
              {product.images?.back && <Image src={product.images.back} alt="Ảnh sau" width={80} height={60} className="object-contain w-20 h-14" />}
              {product.images?.left && <Image src={product.images.left} alt="Ảnh trái" width={80} height={60} className="object-contain w-20 h-14" />}
              {product.images?.right && <Image src={product.images.right} alt="Ảnh phải" width={80} height={60} className="object-contain w-20 h-14" />}
            </div>
          </div>
          {/* Main Info */}
          <div className="w-full lg:w-1/2 flex flex-col justify-center">
            <h2 className="text-3xl font-bold text-[#03bb65] mb-4">{product.name}</h2>
            {/* Giá bán và model */}
            ...
            <span className="font-bold text-[#03bb65] text-2xl mt-4">Giá bán: {product.price ? product.price.toLocaleString() + '₫' : 'N/A'}</span>
            {product.description && (
        ...
        <div className="max-w-5xl mx-auto px-4 py-8">
          {activeTab === "Đặc điểm nổi bật" && (
            <section>
              <h3 className="text-2xl font-bold text-black mb-4">Đặc điểm nổi bật</h3>
              {Array.isArray(product.highlights) && product.highlights.length > 0 ? (
                <ul className="list-disc ml-6 space-y-1 text-black">
                  {product.highlights.map((h: any, idx: number) => (
                    <li key={idx}><b>{h.name}:</b> {h.value}</li>
                  ))}
                </ul>
              ) : (
          ...
          {activeTab === "Thông số kỹ thuật" && (
            <section>
              <h3 className="text-2xl font-bold text-black mb-4">Thông số kỹ thuật</h3>
              {Array.isArray(product.specifications) && product.specifications.length > 0 ? (
                <ul className="list-disc ml-6 space-y-1 text-black">
                  {product.specifications.map((s: any, idx: number) => (
                    <li key={idx}><b>{s.name}:</b> {s.value}</li>
                  ))}
                </ul>
              ) : (
                <p className="text-black">Không có thông tin.</p>
          ...
          {activeTab === "Hình ảnh" && (
            <section>
              ...
              <div className="flex flex-wrap gap-4">
                {product.images?.main && <Image src={product.images.main} alt="Ảnh chính" width={300} height={200} className="rounded-xl object-contain" />}
                {product.images?.front && <Image src={product.images.front} alt="Ảnh trước" width={200} height={140} className="rounded-xl object-contain" />}
                {product.images?.back && <Image src={product.images.back} alt="Ảnh sau" width={200} height={140} className="rounded-xl object-contain" />}
                {product.images?.left && <Image src={product.images.left} alt="Ảnh trái" width={200} height={140} className="rounded-xl object-contain" />}
                {product.images?.right && <Image src={product.images.right} alt="Ảnh phải" width={200} height={140} className="rounded-xl object-contain" />}
              </div>
            </section>
          )}
        </div>
